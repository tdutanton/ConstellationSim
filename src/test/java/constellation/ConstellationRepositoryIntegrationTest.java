...
class ConstellationRepositoryIntegrationTest {
    ...
    class FullLifecycle {
        ...
        void shouldCompleteFullLifecycleSuccessfully() {
            ...
            assertFalse(commSatellite.getState().isActive(), "Спутник должен быть неактивен после создания");
            ...
            assertFalse(commSatellite.getState().isActive(), "Спутник должен деактивироваться при низком заряде");
            ...
            assertNull(repository.constellationByName(constellationName), "Группировка должна быть удалена");
        }
    }
    ...
    class MultipleConstellations {
        ...
        void shouldManageMultipleIndependentConstellations() {
            ...
            SatelliteConstellation secondRetrieved = repository.constellationByName(secondConstellationName);
            ...
            assertEquals(1, firstRetrieved.getSatellites().size(), "В первой группировке должен быть 1 спутник");
            assertEquals(1, secondRetrieved.getSatellites().size(), "Во второй группировке должен быть 1 спутник");

            sat1.activate();
            assertTrue(sat1.getState().isActive(), "Спутник в первой группировке должен быть активен");
            assertFalse(sat2.getState().isActive(), "Спутник во второй группировке должен оставаться неактивным");
        }
    }
